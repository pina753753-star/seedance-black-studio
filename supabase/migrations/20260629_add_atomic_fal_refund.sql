...
CREATE OR REPLACE FUNCTION public.refund_generation_task_atomic(
  p_task_id        uuid,
  p_failure_reason text DEFAULT 'fal_generation_failed',
...
BEGIN
  ...
  -- 4. Do not refund already-completed tasks.
  ...
  -- 5. Idempotency: check for existing refund ledger rows (within task lock).
  ...
  IF v_refund_count > 0 THEN
    -- Ensure task reaches failed status (guards against partial prior failure where
    -- the old JS code refunded but crashed before updating task status).
  ...
  -- 6. Sum original video_generation charges by credit pool (negative amounts only).
  --    Positive amounts (e.g. adjustments) are intentionally excluded.
  FOR v_tx IN
  ...
  -- 7. No charge found: fal ERROR is real, so still mark task failed; skip balance change.
  ...
  -- 8. Lock balance row to serialise balance updates for this user.
  ...
  -- 9. Insert pool-specific refund ledger rows.
  ...
  -- 10. Update balance (balance row lock from step 8 is still held).
...
EXCEPTION
  WHEN unique_violation THEN
    -- A concurrent call raced past the idempotency check (step 5) and inserted
    -- a refund row first. The task FOR UPDATE (step 1) prevents this in normal
    -- flow; this handler is a final safety net.
    RETURN jsonb_build_object(
      'ok', true,
      'refunded', false,
      'already_refunded', true,
  ...
  WHEN OTHERS THEN
    -- Re-raise any other exception to trigger full transaction rollback.
    RAISE;
END;
$$;

-- Step 3: Lock down execution permissions.
REVOKE ALL ON FUNCTION public.refund_generation_task_atomic(uuid, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.refund_generation_task_atomic(uuid, text, text) FROM anon;
REVOKE ALL ON FUNCTION public.refund_generation_task_atomic(uuid, text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.refund_generation_task_atomic(uuid, text, text) TO service_role;

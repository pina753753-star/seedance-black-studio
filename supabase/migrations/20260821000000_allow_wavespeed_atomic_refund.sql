-- Allow refund_generation_task_atomic to service WaveSpeed tasks in addition to fal and OpenRouter.
--
-- Context: Seedance 2.5 1080p tasks use WaveSpeed while the existing atomic
-- refund function accepts only fal and OpenRouter. This migration extends only
-- the provider guard. No other logic in the function changes: the same
...
-- of provider.
--
-- Rollback: re-apply 20260710_allow_openrouter_atomic_refund.sql's CREATE OR
-- REPLACE FUNCTION body (restores the fal/OpenRouter-only guard).
...
BEGIN
  ...
  --    WaveSpeed is accepted for Seedance 2.5 1080p task refunds.
  IF v_api_provider NOT IN ('fal', 'openrouter', 'wavespeed') THEN

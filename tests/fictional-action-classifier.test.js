...
test('ハード矛盾は1回だけ再判定し、整合すれば許可', async (t) => {
  ...
  assert.equal(
    receivedPrompts[1].includes('Do not assume that a human person with a concrete minor indicator is an adult.'),
    true
  );
...
test('二次判定指示文はアニメ的な外見だけで未成年と断定しない', async () => {
  ...
  assert.equal(
    sentPrompt.includes(
      'Do not infer that a character is a minor solely from stylized anime features or ambiguous youthful wording'
    ),
    true
  );
...
test('二次判定指示文は明示された非人間を外見だけで未成年扱いしない', async () => {
  ...
  assert.equal(
    sentPrompt.includes(
      'Trust explicit adult or non-human descriptions in the user prompt.'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'An original fictional spirit with a youthful anime design raises a magical staff.'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'Set adult_or_nonhuman_only true because the character is explicitly non-human and no minor or child status is stated.'
    ),
    true
  );
...
test('二次判定指示文は蚊・害虫のコミカルな対峙を非グラフィックの例として明文化する', async () => {
  ...
  assert.equal(
    sentPrompt.includes(
      'an ordinary original fictional anime character with no concrete minor indicator'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'may comically swat at, chase, or battle a mosquito or ordinary pest'
    ),
    true
  );

  assert.equal(
    sentPrompt.includes('this is ordinarily non-graphic action'),
    true
  );
...
test('二次判定指示文は明示された成人記述を信頼する', async () => {
  ...
  assert.equal(
    sentPrompt.includes(
      'adult_or_nonhuman_only should be true unless the prompt also contains a clear and conflicting minor or child status.'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'An adult woman from the reference character sheet comically battles a mosquito in a fictional anime scene.'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'Set adult_or_nonhuman_only true because the character is explicitly described as an adult woman'
    ),
    true
  );
});

test('二次判定指示文は年齢不詳の一般的なオリジナルアニメキャラクターを一律falseにしない', async () => {
  const sentPrompt = await captureClassifierPrompt(
    'オリジナルアニメキャラクターが架空の部屋で蚊を追いかける'
  );

  assert.equal(
    sentPrompt.includes(
      'A clearly fictional original anime, manga, cartoon, game, or illustrated character may satisfy adult_or_nonhuman_only'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'Do not require an exact numeric adult age for an ordinary original fictional anime character'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'adult_or_nonhuman_only may be true. Do not infer minor status from anime style alone.'
    ),
    true
  );
...
test('二次判定指示文は複数人物のうち1人でも明確な未成年なら全体をfalseにする', async () => {
  const sentPrompt = await captureClassifierPrompt(
    '成人女性と12歳の妖精が一緒に戦う'
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'For scenes with multiple relevant characters'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'An adult woman and a 12-year-old fairy fight together.'
    ),
    true
  );
  ...
  assert.equal(
    sentPrompt.includes(
      'adult_or_nonhuman_only must be false because one relevant character has an explicit minor indicator.'
    ),
    true
  );
});

test('再判定指示文でも成人記述を信頼し年齢不詳だけで未成年扱いしない', async () => {
  let callCount = 0;
  const receivedPrompts = [];

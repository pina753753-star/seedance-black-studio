...
// ── Storyboard image analysis (routed from /api/storyboard via vercel.json) ──

const STORYBOARD_SYSTEM_PROMPT = `あなたは絵コンテ解析の専門家です。
...
async function handleStoryboard(req, res) {
  ...
  if (!allowed.includes(mediaType)) return res.status(400).json({ ok: false, error: `mediaType must be one of: ${allowed.join(', ')}` });
  let response, data;
  try {
  ...
  if (!response.ok) return res.status(502).json({ ok: false, error: `OpenRouter error ${response.status}: ${JSON.stringify(data).slice(0,300)}` });

...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(200).json({ ok: true, endpoint: '/api/ensure-user-credits', method: 'POST' });
  }
  ...
  try {
    ...
    if (existing?.[0]) {
      return res.status(200).json({ ok: true, created: false, balance: existing[0] });
    }
    ...
    return res.status(200).json({ ok: true, created: true, balance: rows?.[0] || null });
  } catch (error) {

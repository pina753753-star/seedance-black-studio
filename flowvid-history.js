(function(){
  ...
  function normalize(row){const url=row?.video_url||row?.video_uri||row?.src||row?.url||'';if(!/^https?:\/\//i.test(url))return null;if(/openrouter\.ai\/api\/v1\/videos\/[^/?#]+\/?(?:[?#].*)?$/i.test(url))return null;return{url,jobId:row?.job_id||row?.jobId||row?.id||'',prompt:String(row?.prompt||row?.settings?.prompt||'').trim(),mode:row?.mode||'',duration:row?.duration_seconds||row?.duration||5,aspect:row?.aspect_ratio||row?.aspectRatio||'9:16',createdAt:row?.created_at||row?.createdAt||''}}

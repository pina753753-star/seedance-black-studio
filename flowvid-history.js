(function(){
  ...
  function modelLabel(v,provider){v=String(v||'').trim();provider=String(provider||'').trim();return v==='bytedance/seedance-2.5-standard'?'Seedance 2.5':v==='bytedance/seedance-2.5'?(provider==='openrouter'?'Seedance 2.5':provider==='wavespeed'?'Seedance 2.5 Turbo':'Seedance 2.5（旧履歴）'):v==='bytedance/seedance-2.0-fast'?'Seedance 2.0 Fast':v==='bytedance/seedance-2.0-lite'?'Seedance 2.0 Lite':v==='bytedance/seedance-2.0'?'Seedance 2.0':v}
  ...
  function inferMode(row){const m=String(row?.mode||'').trim();if(m)return m;const s=row?.settings||{};if(Array.isArray(s.reference_urls)&&s.reference_urls.length)return'reference_to_video';if(s.reference_url||s.referenceUrl)return'reference_to_video';if(s.first_frame_url||s.input_image_url||s.image_url||s.imageUrl||s.inputImageUrl)return'image_to_video';if(row?.reference_url||row?.first_frame_url||row?.input_image_url)return'image_to_video';if(Array.isArray(row?.reference_urls)&&row.reference_urls.length)return'reference_to_video';return''}
  function normalize(row){const url=row?.video_url||row?.video_uri||row?.src||row?.url||'';if(!/^https?:\/\//i.test(url))return null;if(/openrouter\.ai\/api\/v1\/videos\/[^/?#]+\/?(?:[?#].*)?$/i.test(url))return null;return{url,jobId:row?.job_id||row?.jobId||row?.id||'',prompt:String(row?.prompt||row?.settings?.prompt||'').trim(),provider:String(row?.provider||row?.api_provider||row?.settings?.provider||'').trim(),model:String(row?.model||row?.settings?.model||'').trim(),mode:inferMode(row),duration:row?.duration_seconds||row?.duration||5,aspect:row?.aspect_ratio||row?.aspectRatio||'9:16',createdAt:row?.created_at||row?.createdAt||''}}
  ...
  async function remoteSave(){ return false; }
  ...
  function renderHistory(items){const h=$('history');if(!h)return;style();if(!items.length){h.innerHTML='<div class="empty">まだ動画がありません</div>';return}h.innerHTML=items.map(it=>{const job=esc(it.jobId||'');const url=esc(it.url);const promptRaw=String(it.prompt||'').trim();const hasPrompt=Boolean(promptRaw);const prompt=esc(promptRaw);const promptDisplay=hasPrompt?esc(promptRaw.slice(0,72)):'プロンプトがありません';const promptStyle=hasPrompt?'':' style="color:#6b7280;cursor:default"';const modeTag=it.mode?'<span class="fv-chip fv-chip-mode">'+esc(modeLabel(it.mode))+'</span>':'';const meta=modeTag+[modelLabel(it.model,it.provider),it.aspect,(it.duration||5)+'秒',shortDate(it.createdAt)].filter(Boolean).map(v=>'<span class="fv-chip">'+esc(v)+'</span>').join('');const favOn=isFav(job);return '<article class="old" data-job-id="'+job+'"><div class="fv-prompt-wrap"><div class="fv-prompt" data-full-prompt="'+prompt+'"'+promptStyle+'>'+promptDisplay+'</div><div class="fv-meta">'+meta+'</div></div><div class="fv-video-frame" data-open-url="'+url+'"><video muted playsinline preload="auto" src="'+url+'#t=0.2"></video><button class="fv-expand-btn" type="button" data-url="'+url+'" aria-label="全画面で開く">⤢</button></div><div class="fv-actions-row"><a class="fv-action fv-open" href="'+url+'" rel="noreferrer">'+icOpen+' 開く</a><button type="button" class="fv-action fv-save" data-url="'+url+'" data-job-id="'+job+'">'+icDl+' 保存</button><button type="button" class="fv-action fv-copy" data-prompt="'+prompt+'">'+icCopy+' コピー</button></div><div class="fv-actions-row"><button type="button" class="fv-action fv-fav-btn'+(favOn?' on':'')+'" data-job-id="'+job+'">'+(favOn?icStarOn+' お気に入り済み':icStarOff+' お気に入り')+'</button><button type="button" class="fv-action fv-delete-one" data-job-id="'+job+'" data-url="'+url+'">'+icX+' 削除</button></div></article>'}).join('');prepareVideos()}
  let _lastFilterMode;
  // APIから受け取った件数（フィルタ適用前の生の取得件数）と、実際に画面へ
  // 描画される件数（モードフィルタ・非表示指定適用後）の両方を保持し、他
  // スクリプト(flowvid-open-inline.js等)がAPI上限(50件)到達の案内表示に
  // 使えるようにする。表示件数を分けて渡すのは、生の取得件数が50件でも、
  // 現在のモードタブに一致する動画が0件で一覧が空表示になるケースがあり、
  // その場合に「最新50件を表示中です」という矛盾した案内を出さないため。
  // ローカル保存分へのフォールバック時は「APIから何件取れたか」が不明なため、
  // countにnullを渡して「件数不明」を明示する(推測で件数を出さない)。
  function reportFetchedCount(count,displayedCount){
    window.flowvidHistoryFetchedCount=count;
    window.flowvidHistoryDisplayedCount=displayedCount;
    try{window.dispatchEvent(new CustomEvent('flowvid:history-count',{detail:{count,displayedCount}}))}catch(_){}
  }
  async function loadHistory(filterMode){const h=$('history');if(!h)return;_lastFilterMode=filterMode;style();h.innerHTML='<div class="empty">履歴を読み込み中...</div>';try{let _fvToken='';try{const _fvCfg=window.FLOWVID_AUTH||{};if(window.supabase&&_fvCfg.supabaseUrl&&_fvCfg.supabaseAnonKey){if(!window.__flowvidUserClient)window.__flowvidUserClient=window.supabase.createClient(_fvCfg.supabaseUrl,_fvCfg.supabaseAnonKey);const{data:_fvSd}=await window.__flowvidUserClient.auth.getSession();_fvToken=_fvSd?.session?.access_token||''}}catch(_){}const res=await originalFetch('/api/generated-videos?limit=50&t='+Date.now(),{cache:'no-store',headers:_fvToken?{'Authorization':'Bearer '+_fvToken}:{}});const data=await res.json();const fetchedCount=Array.isArray(data?.rows)?data.rows.length:null;const hidden=hiddenSet();let items=(data?.rows||[]).map(normalize).filter(it=>it&&!hidden.has(it.url)&&!hidden.has(it.jobId));if(filterMode)items=items.filter(it=>it.mode===filterMode);renderHistory(items);reportFetchedCount(fetchedCount,items.length)}catch(_){const list=safeJson(localStorage.getItem(HISTORY_KEY),'[]')||[];const hidden2=hiddenSet();let items=list.map(normalize).filter(it=>it&&!hidden2.has(it.url)&&!hidden2.has(it.jobId));if(filterMode)items=items.filter(it=>it.mode===filterMode);renderHistory(items);reportFetchedCount(null,items.length)}}

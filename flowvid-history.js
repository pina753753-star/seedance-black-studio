(function(){
  ...
  function install(){
    ...
    document.addEventListener('click',e=>{const del=e.target?.closest?.('.fv-delete-one');if(del){e.preventDefault();deleteHistory(del.dataset.jobId||'');return}const fav=e.target?.closest?.('.fv-action.fav');if(fav){e.preventDefault();toggleFav(fav.dataset.jobId||'');return}const prompt=e.target?.closest?.('.fv-prompt');if(prompt?.dataset?.fullPrompt){alert(prompt.dataset.fullPrompt);return}const frame=e.target?.closest?.('.fv-video-frame');if(frame?.dataset?.openUrl){window.open(frame.dataset.openUrl,'_blank','noopener')}});

(function(){
  ...
  function install(){
    ...
    document.addEventListener('click',e=>{const save=e.target?.closest?.('.fv-save');if(save){e.preventDefault();e.stopPropagation();saveVideo(save.dataset.url||'',save.dataset.jobId||'',save);return}const del=e.target?.closest?.('.fv-delete-one');if(del){e.preventDefault();deleteHistory(del.dataset.jobId||'',del.dataset.url||'');return}const copy=e.target?.closest?.('.fv-copy');if(copy){e.preventDefault();e.stopPropagation();const u=copy.dataset.url||'';if(u)navigator.clipboard?.writeText(u).then(()=>{const t=copy.textContent;copy.textContent='コピー済';setTimeout(()=>{copy.textContent=t},1500)});return}const fav=e.target?.closest?.('.fv-action.fav');if(fav){e.preventDefault();toggleFav(fav.dataset.jobId||'');return}const prompt=e.target?.closest?.('.fv-prompt');if(prompt){e.preventDefault();e.stopPropagation();togglePromptPop(prompt);return}const pop=e.target?.closest?.('.fv-prompt-pop');if(pop)return;closePromptPops();const frame=e.target?.closest?.('.fv-video-frame');if(frame?.dataset?.openUrl){window.open(frame.dataset.openUrl,'_blank','noopener')}});

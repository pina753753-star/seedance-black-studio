(function(){
  ...
  function modelLabel(v){v=String(v||'').trim();return v==='bytedance/seedance-2.5'?'Seedance 2.5':v==='bytedance/seedance-2.0-fast'?'Seedance 2.0 Fast':v==='bytedance/seedance-2.0-lite'?'Seedance 2.0 Lite':v==='bytedance/seedance-2.0'?'Seedance 2.0':v}
  ...
  function normalize(row){const url=row?.video_url||row?.video_uri||row?.src||row?.url||'';if(!/^https?:\/\//i.test(url))return null;if(/openrouter\.ai\/api\/v1\/videos\/[^/?#]+\/?(?:[?#].*)?$/i.test(url))return null;return{url,jobId:row?.job_id||row?.jobId||row?.id||'',prompt:String(row?.prompt||row?.settings?.prompt||'').trim(),model:String(row?.model||row?.settings?.model||'').trim(),mode:inferMode(row),duration:row?.duration_seconds||row?.duration||5,aspect:row?.aspect_ratio||row?.aspectRatio||'9:16',createdAt:row?.created_at||row?.createdAt||''}}

(function(){
  ...
  let _lastFilterMode;
  // APIから受け取った件数（フィルタ適用前の生の取得件数）を保持し、他スクリプト
  // (flowvid-open-inline.js等)がAPI上限(50件)到達の案内表示に使えるようにする。
  // ローカル保存分へのフォールバック時は「APIから何件取れたか」が不明なため、
  // countにnullを渡して「件数不明」を明示する(推測で件数を出さない)。
  function reportFetchedCount(count){
    window.flowvidHistoryFetchedCount=count;
    try{window.dispatchEvent(new CustomEvent('flowvid:history-count',{detail:{count}}))}catch(_){}
  }

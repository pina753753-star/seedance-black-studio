(function(){
  ...
  function renderHistory(items){const h=$('history');if(!h)return;style();if(!items.length){h.innerHTML='<div class="empty">まだ動画がありません</div>';return}h.innerHTML=items.map(it=>{const job=esc(it.jobId||'');const url=esc(it.url);const prompt=esc(it.prompt||'生成動画');const meta=[modeLabel(it.mode),it.aspect,(it.duration||5)+'秒',shortDate(it.createdAt)].filter(Boolean).map(v=>'<span class="fv-chip">'+esc(v)+'</span>').join('');return '<article class="old" data-job-id="'+job+'"><div class="fv-title-row"><div><div class="fv-prompt" data-full-prompt="'+prompt+'" title="'+prompt+'">'+esc((it.prompt||'生成動画').slice(0,72))+'</div><div class="fv-meta">'+meta+'</div></div><button type="button" class="fv-action fav '+(isFav(job)?'on':'')+'" data-job-id="'+job+'">'+(isFav(job)?'★':'☆')+'</button></div><div class="fv-video-frame" data-open-url="'+url+'"><video muted playsinline preload="auto" src="'+url+'#t=0.2"></video></div><div class="fv-card-actions"><div class="fv-left-actions"><a class="fv-action" href="'+url+'" target="_blank" rel="noreferrer">開く</a><a class="fv-action" href="'+url+'" download>保存</a></div><button type="button" class="fv-action fv-delete-one" data-job-id="'+job+'">削除</button></div></article>'}).join('');prepareVideos()}
  ...
  function showFullPrompt(el){const text=el?.dataset?.fullPrompt;if(text)alert(text)}
  function install(){
    style();loadHistory();
    ...
    document.addEventListener('touchstart',e=>{const prompt=e.target?.closest?.('.fv-prompt');if(!prompt)return;clearPromptTimer();promptTimer=setTimeout(()=>{promptTimer=null;showFullPrompt(prompt)},520)},{passive:true});
    document.addEventListener('touchend',clearPromptTimer,{passive:true});
    document.addEventListener('touchmove',clearPromptTimer,{passive:true});
    document.addEventListener('touchcancel',clearPromptTimer,{passive:true});
    document.addEventListener('contextmenu',e=>{const prompt=e.target?.closest?.('.fv-prompt');if(prompt?.dataset?.fullPrompt){e.preventDefault();showFullPrompt(prompt)}});
    document.addEventListener('click',e=>{const del=e.target?.closest?.('.fv-delete-one');if(del){e.preventDefault();deleteHistory(del.dataset.jobId||'');return}const fav=e.target?.closest?.('.fv-action.fav');if(fav){e.preventDefault();toggleFav(fav.dataset.jobId||'');return}const frame=e.target?.closest?.('.fv-video-frame');if(frame?.dataset?.openUrl){window.open(frame.dataset.openUrl,'_blank','noopener')}});

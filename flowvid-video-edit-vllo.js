(function(){
  ...
  function install(){
    ...
    style.textContent=`
      ...
      .ve-vllo-trim-head-right{display:flex;align-items:center;gap:8px}
      .ve-vllo-delmode-toggle{border:1px solid rgba(251,113,133,.35);background:#241820;color:#fca5a5;border-radius:9px;height:28px;padding:0 9px;font-size:10px;font-weight:800;white-space:nowrap}
      .ve-vllo-delmode-toggle.on{background:linear-gradient(90deg,#fca5a5,#fb7185);color:#050506;border-color:transparent}
      .ve-vllo-trim-delete-window{position:absolute;top:0;bottom:0;background:rgba(251,113,133,.32);border-left:2px solid #fca5a5;border-right:2px solid #fca5a5;pointer-events:none}
      .ve-vllo-handle.delstart:before{background:#fca5a5}
      .ve-vllo-handle.delend:before{background:#fb7185}
      .ve-vllo-delmode-actions{display:flex;margin-top:8px}
      .ve-vllo-delmode-confirm{width:100%;min-height:40px;border:1px solid rgba(251,113,133,.4);background:linear-gradient(90deg,#fca5a5,#fb7185);color:#050506;border-radius:10px;font-size:12px;font-weight:900}
      .ve-vllo-delmode-error{color:#fca5a5;font-size:11px;font-weight:700;margin-top:6px}
    ...
    // 本体版(generate-prod.htmlのveSeekPreview/_veSeekLast)と同じ間引き。
    // pointermoveはドラッグ中に高頻度で発火し、その都度video.currentTimeへの
    // 代入(→重いシーク処理)を積み重ねると、実際の描画がドラッグ位置に追いつかず
    // 遅延して見える。clipIdごとに直近シーク時刻を記録し、40ms未満の連続呼び出し
    // はスキップする。force===trueの場合(ドラッグ確定時=pointerup)は間引きを
    // 無視し、必ず最新位置へシークする。
    const _veVlloSeekLast={}; // clipId -> 直近シーク時刻(ms)
    function throttledSeekPreview(clip,time,force){
      const clipId=clip.clipId;
      const now=Date.now();
      if(!force&&_veVlloSeekLast[clipId]&&now-_veVlloSeekLast[clipId]<40)return;
      _veVlloSeekLast[clipId]=now;
      seekPreview(clip,time);
    }
    ...
    function trimModeToggleBtn(c){return '<button type="button" class="ve-vllo-delmode-toggle'+(c.deleteMode?' on':'')+'" data-ve-delmode-toggle="'+esc(c.clipId)+'">'+(c.deleteMode?'↩ トリムに戻す':'✂ 区間削除')+'</button>'}
    function deleteMarkup(c,i){const d=Math.max(.2,c.duration||5);const ds=Number.isFinite(c.delStart)?c.delStart:d/3;const de=Number.isFinite(c.delEnd)?c.delEnd:d*2/3;const sp=ds/d*100,ep=de/d*100;return '<div class="ve-vllo-trim" data-ve-clip-id="'+esc(c.clipId)+'" data-ve-video-id="'+esc(c.videoId)+'"><div class="ve-vllo-trim-head"><span class="ve-vllo-trim-title">クリップ '+(i+1)+' の区間を削除</span>'+trimModeToggleBtn(c)+'</div><div class="ve-vllo-trim-track ve-vllo-trim-track-delete"><div data-ve-filmstrip-fallback></div><div data-ve-filmstrip></div><div class="ve-vllo-trim-delete-window" data-window style="left:'+sp+'%;right:'+(100-ep)+'%"></div><button type="button" class="ve-vllo-handle delstart" data-handle="delStart" style="left:'+sp+'%" aria-label="削除開始位置"></button><button type="button" class="ve-vllo-handle delend" data-handle="delEnd" style="left:'+ep+'%" aria-label="削除終了位置"></button></div><div class="ve-vllo-time-row"><span data-delstart>削除: '+veFmtTime(ds)+'</span><span class="used" data-deldur-row>削除される長さ: '+veFmtTime(Math.max(0,de-ds))+'</span><span data-delend>〜 '+veFmtTime(de)+'</span></div><div class="ve-vllo-delmode-actions"><button type="button" class="ve-vllo-delmode-confirm" data-ve-delconfirm="'+esc(c.clipId)+'">この区間を削除</button></div><div class="ve-vllo-delmode-error" data-ve-delerror style="display:none"></div></div>'}
    function trimMarkup(c){const d=Math.max(.2,c.duration||5),sp=c.start/d*100,ep=c.end/d*100,i=veSelected.findIndex(x=>x.clipId===c.clipId);if(c.deleteMode)return deleteMarkup(c,i);return '<div class="ve-vllo-trim" data-ve-clip-id="'+esc(c.clipId)+'" data-ve-video-id="'+esc(c.videoId)+'"><div class="ve-vllo-trim-head"><span class="ve-vllo-trim-title">クリップ '+(i+1)+' をトリミング</span><div class="ve-vllo-trim-head-right"><span class="ve-vllo-trim-title" data-used>使用 '+veFmtTime(c.end-c.start)+'</span>'+trimModeToggleBtn(c)+'</div></div><div class="ve-vllo-trim-track"><div data-ve-filmstrip-fallback></div><div data-ve-filmstrip></div><div class="ve-vllo-trim-shade" data-left-shade style="left:0;width:'+sp+'%"></div><div class="ve-vllo-trim-window" data-window style="left:'+sp+'%;right:'+(100-ep)+'%"></div><div class="ve-vllo-trim-shade" data-right-shade style="left:'+ep+'%;right:0"></div><button type="button" class="ve-vllo-handle start" data-handle="start" style="left:'+sp+'%" aria-label="開始位置"></button><button type="button" class="ve-vllo-handle end" data-handle="end" style="left:'+ep+'%" aria-label="終了位置"></button></div><div class="ve-vllo-time-row"><span data-start>'+veFmtTime(c.start)+'</span><span class="used" data-used-row>使用: '+veFmtTime(c.end-c.start)+'</span><span data-end>'+veFmtTime(c.end)+'</span></div></div>'}
    ...
    // トリム(syncTrimDom)と対になる、区間削除モードのドラッグ中DOM同期。
    // 確定検証(最小区間長・上限チェック)はveApplyDeleteRange(確定ボタン)側で行う。
    function syncDeleteTrimDom(c){const root=document.querySelector('#veVlloTrim [data-ve-clip-id="'+CSS.escape(c.clipId)+'"]');if(!root)return;const d=Math.max(.2,c.duration||5);const ds=Number.isFinite(c.delStart)?c.delStart:0,de=Number.isFinite(c.delEnd)?c.delEnd:d;const sp=ds/d*100,ep=de/d*100;const startHandle=root.querySelector('[data-handle="delStart"]');if(startHandle)startHandle.style.left=sp+'%';const endHandle=root.querySelector('[data-handle="delEnd"]');if(endHandle)endHandle.style.left=ep+'%';const win=root.querySelector('[data-window]');if(win){win.style.left=sp+'%';win.style.right=(100-ep)+'%'}const ls=root.querySelector('[data-delstart]');if(ls)ls.textContent='削除: '+veFmtTime(ds);const le=root.querySelector('[data-delend]');if(le)le.textContent='〜 '+veFmtTime(de);const dl=root.querySelector('[data-deldur-row]');if(dl)dl.textContent='削除される長さ: '+veFmtTime(Math.max(0,de-ds))}
    function renderTrim(c){const host=document.getElementById('veVlloTrim');host.innerHTML=c?trimMarkup(c):'';if(c){if(c.deleteMode)installDeleteDrag(c.clipId);else installTrimDrag(c.clipId)}}
    // apply(clientX, commit, force): commitは「トリム範囲を確定してタイムライン等を
    // 再描画するか」、forceは「間引きを無視してプレビューを必ずシークするか」で、
    // 別々の意味を持つ。pointerdownはforce=trueで初回シークを確実に行うが、
    // commit=falseのままrenderTimeline()等の確定処理は走らせない(ドラッグ開始時点
    // ではまだ範囲確定していないため)。pointerup(finish)はcommit=trueのみを渡すが、
    // throttledSeekPreview呼び出し側でcommit===trueも強制シークの条件に含めている
    // ため、forceを明示しなくても確定時は必ずシークされる。
    function installTrimDrag(clipId){const root=document.querySelector('#veVlloTrim [data-ve-clip-id="'+CSS.escape(clipId)+'"]');const track=root?.querySelector('.ve-vllo-trim-track');if(!track)return;let kind=null,pointerId=null;const apply=(clientX,commit,force)=>{const clip=getClip(clipId);if(!clip)return;const r=track.getBoundingClientRect();if(!r.width)return;const raw=Math.max(0,Math.min(1,(clientX-r.left)/r.width))*clip.duration;if(kind==='start')clip.start=Math.min(raw,clip.end-.1);else clip.end=Math.max(raw,clip.start+.1);veClampClip(clip,clip.duration);syncTrimDom(clip);throttledSeekPreview(clip,kind==='start'?clip.start:Math.max(clip.start,clip.end-.05),force===true||commit===true);if(commit){renderTimeline();syncFilmstrips();if(typeof veRenderBar==='function')veRenderBar()}};track.addEventListener('pointerdown',e=>{e.preventDefault();const clip=getClip(clipId);if(!clip)return;const handle=e.target.closest('[data-handle]');if(handle)kind=handle.dataset.handle;else{const r=track.getBoundingClientRect(),t=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width))*clip.duration;kind=Math.abs(t-clip.start)<=Math.abs(t-clip.end)?'start':'end'}pointerId=e.pointerId;track.setPointerCapture?.(pointerId);apply(e.clientX,false,true)});track.addEventListener('pointermove',e=>{if(pointerId!==e.pointerId)return;e.preventDefault();apply(e.clientX,false,false)});const finish=e=>{if(pointerId!==e.pointerId)return;e.preventDefault();apply(e.clientX,true);try{track.releasePointerCapture(pointerId)}catch(_){}pointerId=null;kind=null};track.addEventListener('pointerup',finish);track.addEventListener('pointercancel',e=>{if(pointerId===e.pointerId){pointerId=null;kind=null}})}
    // 区間削除モードのドラッグ操作。installTrimDragと対だが、書き込み先がdelStart/delEndで、
    // ドラッグ中は確定させない(確定は「この区間を削除」ボタン=veApplyDeleteRangeのみ)。
    // apply(clientX, commit, force): installTrimDragと同じ意味分離。pointerdownは
    // force=trueで初回シークを確実に行い、commit=falseのまま(区間削除モードは
    // そもそもドラッグ中に確定処理を持たない)。
    ...
    shell.onclick=e=>{
      ...
      const delToggle=e.target.closest('[data-ve-delmode-toggle]');
      if(delToggle){stopSequence(false);veToggleDeleteMode(delToggle.getAttribute('data-ve-delmode-toggle'));renderAll();return}
      const delConfirm=e.target.closest('[data-ve-delconfirm]');
      // veApplyDeleteRange内部でveRenderList()(=このファイルが上書きしたrenderAll()込みの
      // バージョン)とveRenderBar()を呼ぶため、ここでの再描画は不要。
      if(delConfirm){stopSequence(false);veApplyDeleteRange(delConfirm.getAttribute('data-ve-delconfirm'));return}

(function(){
  ...
  function install(){
    ...
    function installDeleteDrag(clipId){const root=document.querySelector('#veVlloTrim [data-ve-clip-id="'+CSS.escape(clipId)+'"]');const track=root?.querySelector('.ve-vllo-trim-track-delete');if(!track)return;let kind=null,pointerId=null;const apply=clientX=>{const clip=getClip(clipId);if(!clip)return;const r=track.getBoundingClientRect();if(!r.width)return;const dur=Math.max(.2,clip.duration||5);const raw=Math.max(0,Math.min(1,(clientX-r.left)/r.width))*dur;let ds=Number.isFinite(clip.delStart)?clip.delStart:dur/3;let de=Number.isFinite(clip.delEnd)?clip.delEnd:dur*2/3;if(kind==='delStart')ds=Math.min(raw,de);else de=Math.max(raw,ds);clip.delStart=Math.max(0,Math.min(ds,dur));clip.delEnd=Math.max(0,Math.min(de,dur));syncDeleteTrimDom(clip);seekPreview(clip,kind==='delStart'?clip.delStart:clip.delEnd)};track.addEventListener('pointerdown',e=>{e.preventDefault();const clip=getClip(clipId);if(!clip)return;const handle=e.target.closest('[data-handle]');if(handle)kind=handle.dataset.handle;else{const dur=Math.max(.2,clip.duration||5);const r=track.getBoundingClientRect();const t=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width))*dur;const ds=Number.isFinite(clip.delStart)?clip.delStart:dur/3;const de=Number.isFinite(clip.delEnd)?clip.delEnd:dur*2/3;kind=Math.abs(t-ds)<=Math.abs(t-de)?'delStart':'delEnd'}pointerId=e.pointerId;track.setPointerCapture?.(pointerId);apply(e.clientX)});track.addEventListener('pointermove',e=>{if(pointerId!==e.pointerId)return;e.preventDefault();apply(e.clientX)});const finish=e=>{if(pointerId!==e.pointerId)return;e.preventDefault();apply(e.clientX);try{track.releasePointerCapture(pointerId)}catch(_){}pointerId=null;kind=null};track.addEventListener('pointerup',finish);track.addEventListener('pointercancel',e=>{if(pointerId===e.pointerId){pointerId=null;kind=null}})}

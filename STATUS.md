...
- **NSFW・違法コンテンツの技術的検知・フィルタリング**: **OpenAI Moderation APIによる生成前チェックを `api/_lib/seedance-start.js` のSupabase JWT認証成功後・残高確認/タスク作成/クレジット消費/OpenRouter呼び出し前に実装し、PR #82を更新済み。A案（検査失敗時は生成を停止）を採用。本番反映にはVercelのProduction・Preview両方の環境変数 `OPENAI_API_KEY` 設定が必須。未設定の間はマージすると生成機能全体が停止するため、環境変数設定完了を確認してからマージすること。（本番未適用・人間の承認待ち）**

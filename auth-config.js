...
(function flowvidCreditUi(){
  ...
  function injectStyle(){if(document.getElementById('flowvidCreditUiStyle'))return;const style=document.createElement('style');style.id='flowvidCreditUiStyle';style.textContent=`body.flowvid-credit-ui .settings .estimate{display:none!important}body.flowvid-credit-ui #notice{display:none!important}body.flowvid-credit-ui .taskPreview{display:none!important}body.flowvid-credit-ui .flowvid-unfinished-task .taskActions{display:none!important}body.flowvid-credit-ui .flowvid-unfinished-task{padding-bottom:13px!important}body.flowvid-credit-ui #createBtn{display:flex!important;align-items:center!important;justify-content:center!important;gap:10px!important}body.flowvid-credit-ui #createBtn .flowvidCreateCost{display:inline-flex!important;align-items:center!important;gap:4px!important;font-weight:1000!important;opacity:.92!important}body.flowvid-credit-ui #createBtn .flowvidCostIcon{width:22px!important;height:22px!important;border-radius:999px!important;display:inline-grid!important;place-items:center!important;background:#050506!important;color:#fff!important;font-size:13px!important;line-height:1!important}`;document.head.appendChild(style)}
  function markUnfinishedTasks(){document.querySelectorAll('.taskCard').forEach(card=>{const hasVideo=Boolean(card.querySelector('.taskVideoWrap'));card.classList.toggle('flowvid-unfinished-task',!hasVideo)})}
  function apply(){if(!isGeneratePage())return;document.body.classList.add('flowvid-credit-ui');injectStyle();markUnfinishedTasks();document.querySelector('.settings .estimate')?.setAttribute('aria-hidden','true');const modeLabel=document.getElementById('modeLabel');if(modeLabel)modeLabel.textContent=activeModeLabel();const createBtn=document.getElementById('createBtn');if(createBtn&&!createBtn.disabled){createBtn.innerHTML='<span>作成する</span><span class="flowvidCreateCost"><span class="flowvidCostIcon">✦</span>128</span>'}}

...
(function persistAdminViewAcrossReloads(){
  ...
  function hook(){
    ...
    if(!window.showView.__flowvidPersistHooked){
      const original=window.showView;
      window.showView=function(name){const r=original.apply(this,arguments);if(allowed.has(name))remember(name);return r};
      window.showView.__flowvidPersistHooked=true;
    }
    const view=hashView()||savedView();
    if(view){window.showView(view);remember(view)}
    document.querySelectorAll('.drawer .nav button[data-view]').forEach(btn=>{
      if(btn.dataset.flowvidRememberView)return;
      btn.dataset.flowvidRememberView='1';
  ...
  function start(){
    if(!/\/admin\.html$/.test(location.pathname))return;
    let n=0;
    const timer=setInterval(()=>{n++;if(hook()||n>40)clearInterval(timer)},150);
  }
...
(function autoSaveCompletedVeoResults(){
  function findVideoUri(obj){
    ...
    for(const key of Object.keys(obj)){const found=findVideoUri(obj[key]);if(found)return found}
    return'';
  }
  function findOperationName(obj){
    ...
    for(const key of Object.keys(obj)){const found=findOperationName(obj[key]);if(found)return found}
    return'';
  }
  ...
  async function save(el){
    ...
    const payload={
      operationName,videoUri,response:data,
    ...
    try{
      const response=await fetch('/api/generated-videos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
  ...
  function observe(id){
    ...
    if(!el||el.dataset.flowvidAutoSaveObserver)return;
    el.dataset.flowvidAutoSaveObserver='1';
    new MutationObserver(()=>save(el)).observe(el,{childList:true,characterData:true,subtree:true});
    save(el);
  }
...
(function addRunDraftTaskButtons(){
  function extractTaskId(item){
    const code=item.querySelector('button[onclick^="deleteTask"]')?.getAttribute('onclick')||'';
    return code.match(/deleteTask\('([^']+)'\)/)?.[1]||'';
  }
  ...
  async function runTask(taskId,button){
    ...
    try{
      const response=await fetch('/api/run-generation-task',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({taskId})});
      const result=await response.json();
      if(!response.ok||!result.ok)throw new Error(result.error||'Veo実行に失敗しました');
      if(result.operationName)localStorage.setItem('flowvidVeoOperation',result.operationName);
      setTaskStatus(`${JSON.stringify(result,null,2)}\n\n✅ Veo実行を開始しました。operationName が出ています。`);
      const refresh=document.getElementById('refreshTasksBtn');
      if(refresh)setTimeout(()=>refresh.click(),900);
    }catch(error){
  ...
  function enhance(){
    ...
    document.querySelectorAll('#taskList .taskItem').forEach(item=>{
      if(item.dataset.flowvidRunButtonAdded)return;
      const status=item.querySelector('.taskTop span')?.textContent?.trim()||'';
      if(!['draft','queued','pending','failed','error'].includes(status))return;
      const taskId=extractTaskId(item);
      const actions=item.querySelector('.taskActions');
      if(!taskId||!actions)return;
      const btn=document.createElement('button');
      btn.type='button';
      btn.className='taskBtn';
      btn.textContent='Veo実行';
      btn.onclick=()=>runTask(taskId,btn);
...
(function addCurrentUsageEstimate(){
  ...
  function monthRows(rows){
    ...
    return(rows||[]).filter(row=>{const date=row.created_at?new Date(row.created_at):null;return date&&!Number.isNaN(date.getTime())&&date.getFullYear()===year&&date.getMonth()===month});
  }
  function ensureCard(){
    ...
    card.innerHTML=`<h2>今月の利用状況</h2><div class="row"><span>FlowVid記録の利用額</span><strong id="flowvidUsageAmount">読み込み中</strong></div><div class="row"><span>生成回数</span><strong id="flowvidUsageCount">-</strong></div><div class="row"><span>月上限まで残り</span><strong id="flowvidUsageRemaining">-</strong></div><div class="row"><span>上限使用率</span><strong id="flowvidUsagePercent">-</strong></div><div class="mini" id="flowvidUsageNote">generated_videos の credit_cost を集計した目安です。Google AI Studioの実請求額は下のボタンから確認してください。</div><button class="btn secondary" id="refreshUsageEstimateBtn" type="button">利用状況を再読み込み</button>`;
    const first=settings.querySelector('.card');
    first?.nextSibling?first.parentNode.insertBefore(card,first.nextSibling):settings.appendChild(card);
    document.getElementById('refreshUsageEstimateBtn')?.addEventListener('click',loadUsage);
  }
  async function loadUsage(){
    ensureCard();
    const a=document.getElementById('flowvidUsageAmount'),c=document.getElementById('flowvidUsageCount'),r=document.getElementById('flowvidUsageRemaining'),p=document.getElementById('flowvidUsagePercent'),n=document.getElementById('flowvidUsageNote');
    if(!a)return;
    a.textContent='読み込み中';
    try{
      ...
      const completed=monthRows(data.rows||[]).filter(row=>['completed','processing'].includes(String(row.status||'')));
      const total=completed.reduce((sum,row)=>sum+Number(row.credit_cost||0),0);
      a.textContent=yen(total);
      c.textContent=`${completed.length}回`;
      r.textContent=yen(Math.max(monthlyLimit-total,0));
      p.textContent=`${Math.min((total/monthlyLimit)*100,999).toFixed(1)}%`;
      n.textContent=`更新：${new Date().toLocaleString('ja-JP')}。この表示はFlowVid内の記録ベースです。Google AI Studio側の実測額とは数円ずれる可能性があります。`;
    }catch(error){
      a.textContent='取得エラー';c.textContent='-';r.textContent='-';p.textContent='-';
      if(n)n.textContent=error.message||String(error);
    }
  }
  function start(){
    if(!/\/admin\.html$/.test(location.pathname))return;
    ensureCard();loadUsage();
    const nav=document.querySelector('.drawer .nav');
    if(nav&&!nav.dataset.flowvidUsageHooked){
      nav.dataset.flowvidUsageHooked='1';
      nav.addEventListener('click',e=>{if(e.target.closest('button[data-view="settings"]'))setTimeout(loadUsage,400)});
    }
  }
...
(function labelGenerateActionButtons(){
  ...
  function start(){
    ...
    if(list&&!list.dataset.flowvidActionLabelObserver){
      ...
      new MutationObserver(labelActions).observe(list,{childList:true,subtree:true});
    }
...
(function runUserCreateButtonWithVeo(){
  ...
  function currentClient(){
    ...
    if(!window.__flowvidUserClient)window.__flowvidUserClient=window.supabase.createClient(cfg.supabaseUrl,cfg.supabaseAnonKey);
    return window.__flowvidUserClient;
  }
  function refreshTasksSoon(){
    const btn=document.getElementById('loadTasksBtn');
    if(btn){setTimeout(()=>btn.click(),500);setTimeout(()=>btn.click(),2500);setTimeout(()=>btn.click(),7000)}
  }
  ...
  async function pollRememberedOperations(){
    ...
    for(const row of rows){
      ...
      try{
        ...
        if(result.completed){notice('動画が完成しました');refreshTasksSoon()}else kept.push(row);
      }catch(_){kept.push(row)}
    }
  ...
  async function createAndRun(){
    ...
    const createBtn=document.getElementById('createBtn'),originalHTML=createBtn?.innerHTML||'作成する';
    if(createBtn){createBtn.disabled=true;createBtn.textContent='生成開始中…'}
    notice('Veo生成を開始しています…');
    status('タスクを作成しています…');
    try{
      const payload={user_id:user.id,mode:activeMode(),prompt,resolution:'veo',duration_seconds:Number(document.getElementById('duration')?.value||5),aspect_ratio:document.getElementById('aspectRatio')?.value||'9:16',credit_cost:128,status:'draft',api_provider:'veo'};
      const{data:task,error:insertError}=await client.from('generation_tasks').insert(payload).select('id').single();
      if(insertError||!task?.id)throw new Error(insertError?.message||'タスク作成に失敗しました');
      status('タスクを作成しました。Veo生成を開始しています…');
      refreshTasksSoon();
      const response=await fetch('/api/run-generation-task',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({taskId:task.id})});
      const result=await response.json();
  ...
  function start(){
    ...
    const timer=setInterval(()=>{
      attempts++;
      const button=document.getElementById('createBtn');
      if(button){button.dataset.flowvidUserRunHooked='';hookCreateButton()}
...
(function flowvidCreditUi(){
  ...
  function activeModeLabel(){return document.querySelector('.modeTabs button.active')?.textContent?.trim()||'リファレンス'}
  function injectStyle(){
    if(document.getElementById('flowvidCreditUiStyle'))return;
    const style=document.createElement('style');
    style.id='flowvidCreditUiStyle';
    style.textContent=`body.flowvid-credit-ui .settings .estimate{display:none!important}body.flowvid-credit-ui #notice{display:none!important}body.flowvid-credit-ui .taskPreview{display:none!important}body.flowvid-credit-ui .flowvid-unfinished-task{display:none!important}body.flowvid-credit-ui #createBtn{display:flex!important;align-items:center!important;justify-content:center!important;gap:10px!important}body.flowvid-credit-ui #createBtn .flowvidCreateCost{display:inline-flex!important;align-items:center!important;gap:4px!important;font-weight:1000!important;opacity:.92!important}body.flowvid-credit-ui #createBtn .flowvidCostIcon{width:22px!important;height:22px!important;border-radius:999px!important;display:inline-grid!important;place-items:center!important;background:#050506!important;color:#fff!important;font-size:13px!important;line-height:1!important}`;
    document.head.appendChild(style);
  }
  function markUnfinishedTasks(){
  ...
  function apply(){
    ...
    injectStyle();
    markUnfinishedTasks();
    document.querySelector('.settings .estimate')?.setAttribute('aria-hidden','true');
    const modeLabel=document.getElementById('modeLabel');
    if(modeLabel)modeLabel.textContent=activeModeLabel();
